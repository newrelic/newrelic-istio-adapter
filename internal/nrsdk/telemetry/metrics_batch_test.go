...
func TestMetrics(t *testing.T) {
	...
	metrics.Attributes = map[string]interface{}{
		"zip": "zap",
	}
	...
	reqs, err := metrics.NewRequests("my-api-key", "")
	if err != nil {
...
func testBatchJSON(t testing.TB, batch *metricBatch, expect string) {
	...
	reqs, err := batch.NewRequests("apiKey", "")
	if nil != err {
		t.Fatal(err)
	}
	if len(reqs) != 1 {
...
func TestBatchAttributesJSON(t *testing.T) {
	...
	testBatchJSON(t, batch, `[{"common":{"attributes":{"zip":"zap"}},"metrics":[]}]`)
...
func TestMetricRequestURL(t *testing.T) {
	batch := &metricBatch{Metrics: []Metric{Count{}}}
	// Default URL
	reqs, err := batch.NewRequests("apiKey", "")
	if err != nil {
	...
	req := reqs[0]
	if req.Request.URL.String() != "https://metric-api.newrelic.com/metric/v1" {
		t.Error(req.Request.URL.String())
	}
	// Override URL
	reqs, err = batch.NewRequests("apiKey", "https://override.host.com/metric/v1")
	if err != nil {
		t.Error(err)
	}
	if len(reqs) != 1 {
	...
	req = reqs[0]
	if req.Request.URL.String() != "https://override.host.com/metric/v1" {
		t.Error(req.Request.URL.String())
	}
}
